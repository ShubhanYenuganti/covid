...
const getStats = (city) => {
    fetch('https://disease.sh/v3/covid-19/vaccine/coverage/countries?lastdays=7&fullData=false/', requestOptions).
...
const updateText = (data, city) => {
    ...
    let index = 0;
    while (index < info.length && info[index].country != city)
    {
        index++
    }
    var country = "For: " + info[index].country + "\n \n";
    ...
    if (city == null) {
        ...
        document.getElementById('output').value = country + day1 + day2 + day3 + day4 + day5 + day6 + day7;
    }
